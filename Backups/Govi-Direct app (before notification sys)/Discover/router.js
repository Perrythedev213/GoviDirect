...
async function loadPageScripts(
  ...
  for (const script of scripts) {
    ...
    /*
      IMPORTANT:
      Resolve target page scripts relative
      to the target page, not the current page.
    */
    const absoluteUrl =
      new URL(
        src,
...
function loadFarmerListings() {
  ...
  listings.forEach(listing => {
    ...
    card.innerHTML = `
      <div class="relative overflow-hidden rounded-2xl bg-surface-container-low">
        <img
          src="${image}"
          alt="${name}"
        ...
        <div
          class="verified-product-badge hidden absolute top-3 right-3 bg-white/95 backdrop-blur-sm rounded-full px-3 py-1.5 shadow-sm flex items-center gap-1.5"
        >
      ...
      <div class="pt-4">
        ...
        <button
          type="button"
          class="add-btn mt-4 w-full rounded-xl bg-primary text-white py-2.5 font-semibold"
        >
          Add to cart
        </button>
      </div>
